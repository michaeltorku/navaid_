...
class StereoPair(object):
    ...
    def get_frames(self):
        ...
        images = [capture.read()[1] for capture in self.captures]
        # rescale down.
        # images = [cv2.resize(img, None, fx=0.5, fy=0.5) for img in images]
        return images
    ...
    def get_chessboard(self, columns, rows, show=False):
        ...
        while not all(found_chessboard):
            ...
            for i, frame in enumerate(frames):
                (found_chessboard[i],corners) = cv2.findChessboardCorners(frame, (columns, rows))
...
class CalibratedPair(StereoPair):
    ...
    def get_point_cloud(self, pair):
        ...
        # colors = cv2.cvtColor(pair[0], cv2.COLOR_GRAY2RGB)
        return PointCloud(points, colors), disparity
